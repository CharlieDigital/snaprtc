...
import { SnapRTCOptions } from './models';
 ...
 */
export async function snaprtcInit(options: SnapRTCOptions) {
  ...
  signalConnection.on("signalStart", () => {
    ...
    peer.on('error', err => console.error('[SnapRTC error]', err))
